...
<body>
  ...
  <p class="" style="font-size:12px; margin-top:1px;"><strong>Gethan ID: <?php print $user_id; ?></strong></p>
...
<a href="logout.php"><i class="fa-power-off fa"></i> Log Out</a><br><br>
  ...
  <a onClick="openCity('London')" class="notification"><span><i class="fa-book fa"></i> Start Learning</span> <span class="badge"><?php print $t_batch; ?></span> </a>
  ...
  <div class="col-md-12 py-4" style="padding-left:2em;"> 
  	<div id="London" class="city">
		...
		<div class="card" align="left" style="border-radius:13px;">
			<div class="card-body" align="left">
  				<h4><?php print $learn['batch']; ?></h4>
				<div class="row">
				<div class="col-md-3"><img src="banner/<?php print $learn['banner']; ?>" style="width: 200px; height: 100px;"></div>
				<div class="col-md-5" style="border-right:1px solid #666666; padding:;margin: auto; height:80px;"><strong><?php print $learn['course']; ?></strong><br>
	
	<?php
	//this section compute the progress bar
	
	$pb = mysqli_query($conn,"select * from subjects where course_id='".$learn['course_id']."'");
	
	if(mysqli_num_rows($pb) < 1){ 
	
	$min = "";
	
	}else{
	
		$cont = mysqli_fetch_array($pb)['subject_id'];

		$min =  "and subject_id='$cont'";

	}
	
	$cc = mysqli_query($conn,"select * from progress where user_id='$user_id' and course_id='".$learn['course_id']."' $min");
	...
	?>
				<div class="progress">
 					<div class="progress-bar" role="progressbar" aria-valuenow="70" aria-valuemin="0" aria-valuemax="100" style="width:100%">100% Loaded</div>
				</div>
				
				</div>
				<div class="col-md-4" style="padding:;margin: auto; height:70px; "><a href="welcome.php?c=<?php print $learn['course_id']; ?>" class="btn-success btn" style="border-radius: 20px; width:230px;color:#fff;" ><i class="fa-play-circle fa"></i> <?php print $text; ?></a></div>
...
<div id="Lagos" class="city" style="display:none">
  <h2>History</h2>
  <p><?php
$b = mysqli_query($conn, "select * from batch_signup where user_id='$user_id'");
$bcheck = mysqli_num_rows($b);


if($bcheck < 1){
print "<div class='alert alert-danger'><i class='fa fa-info-circle'></i> No History </div>";
...
<?php
$color="1";
$bs = mysqli_query($conn, "select * from batch_signup where user_id='$user_id'");
	...
	?>
		...
		<td><?php print $brow['batch'];?></td>
		<td><?php if(empty($brow['course_completed'])){ print "Still in Progress"; }else{ print $brow['course_completed']; } ?></td>
		<td><?php print $brow['date_signed'];?></td>
		<td><a href="" class="btn-primary btn"><i class="fa-play fa"></i> Study</a></td>
	</tr>
	<?php
	}
	?>
