...
//subject pagination to access next subject
//this section insert into progress

$ck = mysqli_query($conn,"select * from progress where user_id='$user_id' and course_id='$cid' and subject_id='$sub'");
	
	if(mysqli_num_rows($ck) > 0){
	
	}else{
$inn = mysqli_query($conn,"insert into progress (user_id,course_id,subject_id) values ('$user_id','$cid','$sub')");
...
$vio = mysqli_query($conn,"select * from subjects where course_id='$cid' order by id asc LIMIT $page_first_result, $results_per_page");
//$po = mysqli_fetch_array($viod)['subject_id'];

//$vio = mysqli_query($conn,"select * from subjects where subject_id='$sub' LIMIT $page_first_result, $results_per_page");
$v = mysqli_fetch_array($vio);
//$subj = $v['subject_id'];

$number_of_result = mysqli_num_rows($vio);
...
<a href="logout.php"><i class="fa-power-off fa"></i> Log Out</a><br><br>
  ...
  <div class="col-md-12 py-4" style="background: #F2F9FF;">
						<h2 style="padding-left:1em;"><?php print $v['subject']; ?></h2>
						
						</div>
  <div class="col-md-12 py-4" style="padding-left:2em;"> 
  	<div id="London" class="city">
  		<div class="col-md-11 sto row">
		...
		<div class="card col-md-12" align="left" style="border-radius:13px;">
			<div class="card-body" align="left">

  				<p> 
				<video width="100%" height="50%" controls id='myVideo'>
...
Your browser does not support the video tag.
	 ...
	 $class = "none";
		...
		$np = $n + 1;
			...
			$cos = mysqli_query($conn, "select * from subjects where course_id='$cid' and subject_id !='$sub' order by id asc limit 1 ");
		
			$nex = mysqli_fetch_array($cos);
			
			$vom = mysqli_query($conn,"select * from subjects where course_id='$cid' order by id asc");
			
			$number_of_result = mysqli_num_rows($vom);
			
					if($np > $number_of_result){
					
					$button ="learning.php?sub=".$v['subject_id']."&course_id=".$nex['course_id']."&page=end";
					
					$text = "Class Ended";
					
				//	$top = "End of Class. Kindly go back to dashboard";
					
					}else{

		$button = "learning.php?sub=".$v['subject_id']."&course_id=".$nex['course_id']."&page=$np";
		$text = "Next Class >>";
		
	//	$top = "Next Topic: ".$nex['subject'];
		}
	 ...
	 ?>
			...
			<p><?php print $v['subject_note']; ?></p>
				<hr>
		
  				<p> 
				
		
		<div align="right" id="tvc" style="display:none;">
		<span  style=" font-size:18px;"><strong><?php print $top; ?></strong><hr></span>
			 <a href="<?php print $button; ?>" style="width: 178px; display:none;" class="btn btn-danger enableIfFinish" id="btnk" ><?php print $text; ?></a>
...
video.addEventListener("ended", function() {
   ...
   var x = document.getElementById("btnk");
   var to = document.getElementById("tvc");
